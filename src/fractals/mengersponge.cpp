...
class MengerSponge : public ShaderProgram {

    public:

        std::string vertex() override {
            return std::string(MengerSpongeCode::vertex);
        }

        std::string fragment() override {
            return std::string(MengerSpongeCode::fragment);
        }

};

REGISTER_SHADER_PROGRAM(Fractal, MengerSponge);